...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        evaluated_first = scheme_eval(first, env)
        evaluated_rest = map_link(lambda x: scheme_eval(x, env), rest)
        if scheme_procedurep(evaluated_first):
            return scheme_apply(evaluated_first, evaluated_rest, env)
        else:
            raise SchemeError('{} is not callable'.format(expr))
        # END PROBLEM 3
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        lambda_env = procedure.env
        frame = lambda_env.make_child_frame(procedure.formals, args)
        ...
        "*** YOUR CODE HERE ***"
        mu_env = env
        frame = mu_env.make_child_frame(procedure.formals, args)
        return eval_all(procedure.body, frame)
        # END PROBLEM 11
    else:
...
def eval_all(expressions, env):
    ...
    if expressions is Link.empty:
        return
    expr = expressions
    while expr.rest is not Link.empty:
